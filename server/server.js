const express = require('express');
const fs = require('fs');
const cartRouter = require('./cartRouter');
const app = express();

app.use(express.json());
app.use('/', express.static('./public'));
app.use('/api/cart', cartRouter);
// app.use('/api/productTest', productRouter);

app.get('/api/products', (req, res) => {
    fs.readFile('./server/db/products.json', 'utf-8', (err, data) => {
        if (err) {
            res.send(JSON.stringify({result: 0, text: err}));
            // res.sendStatus(404, JSON.stringify({result: 0, text: err}));
        } else {
            res.send(data);
        }
    });
});
// ----------------------

app.get('/api/cartprod', (req, res) => {
    fs.readFile('./server/db/productCart.json', 'utf-8', (err, data) => {
        if (err) res.send(JSON.stringify({ result: 0, err }));
        else{  res.send(data);
            console.log('получили данные');
        };
    });
});

app.post('/api/cartprod', (req, res) => {
    fs.writeFile('./server/db/productCart.json', JSON.stringify(req.body), (err) => {
        if (err) res.end(JSON.stringify({ result: 0, err }));
        else res.end(JSON.stringify({ result: 1 }));
    });
    console.log('передали данные');
});

// -------------------
const port = process.env.PORT || 1234;

app.listen(port, () => {
    console.log(`Server started at port ${port}`);
});

// app.get(); // READ
// app.post(); // CREATE
// app.put(); // UPDATE
// app.delete(); // DELETE
